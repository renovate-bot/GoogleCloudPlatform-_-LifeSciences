...
"""Boltz2 Inference Pipeline with configurable FLEX_START scheduling.
...
def create_boltz2_inference_pipeline(strategy: str = "STANDARD"):
    """Factory function to create BOLTZ2 pipeline with specified strategy.
    ...
    @dsl.pipeline(
        name="boltz2-inference-pipeline",
        description="Boltz2 structure prediction with configurable scheduling.",
    )
    def boltz2_inference_pipeline(
        query_json_path: str,
        project: str,
        region: str,
        nfs_cache_path: str,
...
boltz2_inference_pipeline = create_boltz2_inference_pipeline("STANDARD")
