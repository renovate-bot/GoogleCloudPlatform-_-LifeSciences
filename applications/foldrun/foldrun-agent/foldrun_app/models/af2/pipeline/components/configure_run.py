...
@dsl.component(
    base_image=config.ALPHAFOLD_COMPONENTS_IMAGE, packages_to_install=["google-cloud-storage"]
)
def configure_run(
    sequence_path: str,
    model_preset: str,
    ...
    from alphafold.data import parsers  # type: ignore
    from alphafold.model import config as alphafold_config  # type: ignore
    ...
    if len(seqs) != 1 and model_preset != "multimer":
        raise ValueError(
            f"More than one sequence found in {sequence_path}. Unsupported for monomer predictions."
        )

    models = alphafold_config.MODEL_PRESETS[model_preset]
    ...
    for model_name in models:
        for i in range(num_predictions_per_model):
            model_runners.append(
                {"prediction_index": i, "model_name": model_name, "random_seed": random_seed}
            )
            random_seed += 1
    ...
    output = namedtuple(
        "ConfigureRunOutputs", ["model_runners", "run_multimer_system", "num_ensemble"]
    )

    return output(model_runners, run_multimer_system, num_ensemble)
