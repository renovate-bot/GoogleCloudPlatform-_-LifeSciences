...
BFD_PATH = os.getenv("BFD_PATH", "bfd/bfd_metaclust_clu_complete_id30_c90_final_seq.sorted_opt")
SMALL_BFD_PATH = os.getenv("SMALL_BFD_PATH", "small_bfd/bfd-first_non_consensus_sequences.fasta")
...
# MMseqs2 database paths (created by download_database tool alongside FASTA files)
UNIREF90_MMSEQS_PATH = os.getenv("UNIREF90_MMSEQS_PATH", "uniref90/uniref90_mmseqs")
MGNIFY_MMSEQS_PATH = os.getenv("MGNIFY_MMSEQS_PATH", "mgnify/mgnify_mmseqs")
SMALL_BFD_MMSEQS_PATH = os.getenv("SMALL_BFD_MMSEQS_PATH", "small_bfd/small_bfd_mmseqs")

# Machine type for MMseqs2 GPU data pipeline
MMSEQS2_DATA_PIPELINE_MACHINE_TYPE = os.getenv(
    "MMSEQS2_DATA_PIPELINE_MACHINE_TYPE", "g2-standard-12"
)
MMSEQS2_ACCELERATOR_TYPE = os.getenv("MMSEQS2_ACCELERATOR_TYPE", "NVIDIA_L4")
MMSEQS2_ACCELERATOR_COUNT = int(os.getenv("MMSEQS2_ACCELERATOR_COUNT", "1"))

DATA_PIPELINE_MACHINE_TYPE = os.getenv("DATA_PIPELINE_MACHINE_TYPE", "c2-standard-16")
...
# DWS max wait per GPU task, in hours. Converted to seconds at pipeline compile time.
# FLEX_START uses this value; STANDARD uses 24h (hardcoded minimum).
DWS_MAX_WAIT_HOURS = int(os.getenv("DWS_MAX_WAIT_HOURS", "168"))  # 7 days default

XLA_PYTHON_CLIENT_MEM_FRACTION = os.getenv("XLA_PYTHON_CLIENT_MEM_FRACTION", "4.0")
