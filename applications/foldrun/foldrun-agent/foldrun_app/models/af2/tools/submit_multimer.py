...
"""Tool for submitting multimer AlphaFold2 predictions."""
...
class AF2SubmitMultimerTool(AF2Tool):
    """Tool for submitting multimer AlphaFold2 predictions."""
    ...
    def run(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            arguments: {
                'sequence': FASTA content or file path (must have multiple chains),
                ...
                'num_predictions_per_model': Number of predictions per model (1-25),
        ...
        job_name = arguments.get("job_name", f"multimer_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
        ...
        num_predictions_per_model = arguments.get("num_predictions_per_model", 5)
        ...
        if is_monomer:
            raise ValueError(
                "Sequence appears to be monomer. Use submit_af2_monomer_prediction instead."
            )
        ...
        return {
            ...
            "total_predictions": num_predictions_per_model * 5,  # 5 models
